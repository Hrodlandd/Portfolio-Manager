...
public class AddCustomer implements Initializable{
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(sql2);
            ...
            if (rs.next()) {
                max = rs.getString(1);
                System.out.println("Last C_ID: " + (max != null ? max : "null"));
                ...
                if (max != null) {
                    ...
                    if (matcher.find()) {
                        ...
                        try {
                            ...
                            System.err.println("Failed to parse numeric ID: " + numericPart);
                            numericId = 0;
                        }
                    } else {
                    ...
                    System.out.println("No customer records found, starting with ID 1");
                    numericId = 0;
                }
            } else {
                System.out.println("No records in customer table, starting with ID 1");
                numericId = 0;
            }
        } catch (SQLException e) {
            System.err.println("Database error while fetching customer ID: " + e.getMessage());
            numericId = 0;
        }

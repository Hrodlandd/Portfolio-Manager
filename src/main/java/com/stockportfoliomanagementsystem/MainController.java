...
public class MainController implements Initializable {
    ...
    public void onLoginButton(MouseEvent event) throws IOException {
        if (conn == null) {
            ...
            alert.setHeaderText("Database connection is not available");
            alert.setContentText("Please check PostgreSQL and try again.");
            alert.showAndWait();
            return;
        }
        ...
        if((username.isEmpty())||(password.isEmpty())){
            ...
            if (isEmailValid(username)) {
                ...
                try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
                    ...
                    Alert alert = new Alert(Alert.AlertType.ERROR);
                    alert.setTitle("Database Error");
                    alert.setHeaderText("Database query failed");

...
public class AddNewSupplier implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        try {
            ...
            // Initialize to 0 in case no results or parsing fails
            numericId = 0;
            
            if (hasResults && max != null && !max.trim().isEmpty()) {
                try {
                    ...
                    System.err.println("Error parsing supplier ID: " + e.getMessage());
                    numericId = 0; // Reset to default on error
                }
            } else {
                System.out.println("No supplier ID found in the database, starting from 1");
                numericId = 0; // Will be incremented to 1 below
            }
